...
static const char* TAG = "ov2640-camera";
...
static int set_quality(sensor_t *sensor, int qs)
{
    ...
    /* Write QS register */
    ret |= SCCB_Write(sensor->slv_addr, QS, 0x04);
...
static int set_exposure_ctrl(sensor_t *sensor, int enable, int values)
{
    int ret=0;
    uint8_t reg;
    uint16_t manual_exposure_value;
    ...
    if (enable) {
        ...
        reg &= ~COM8_AEC_EN;
        manual_exposure_value = values;
    }

    ret |= SCCB_Write(sensor->slv_addr, COM8, reg);

    if(!enable) {
    	uint8_t reg45 = SCCB_Read(sensor->slv_addr, 0x45); /* Bit[5:0]: AEC[15:10] */
    	uint8_t reg04 = SCCB_Read(sensor->slv_addr, 0x04); /* Bit[1:0]: AEC[1:0] */
    	// AEC (0x10) (AEC[9:2]) does not require read

    	reg04 &= 0xFC; // rip off Bit[1:0]
    	reg04 |= (manual_exposure_value&0x0003);

    	reg45 &= 0xE0; // rip off Bit[5:0]
    	reg45 |= (manual_exposure_value >> 10);

    	ret |= SCCB_Write(sensor->slv_addr, 0x04, reg04);
    	ret |= SCCB_Write(sensor->slv_addr, 0x45, reg45);
    	ret |= SCCB_Write(sensor->slv_addr, 0x10, (manual_exposure_value>>2));
    }
...
int ov2640_init(sensor_t *sensor)
{
    ...
    ESP_LOGI(TAG, "OV2640 Initialization Done.");
    return 0;
}
