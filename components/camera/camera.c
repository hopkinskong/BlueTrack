...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    if(pix_format == PIXFORMAT_JPEG) {
		...
		s_fb_size = 550000; // Force FB size
...
esp_err_t camera_run()
{
    ...
    ESP_LOGI(TAG, "F%d: %dms %dbytes", frame_count, time_ms, camera_get_data_size());
	frame_count++;
    return ESP_OK;
}

void camera_set_h_flip(uint8_t enabled) {
	s_sensor.set_hmirror(&s_sensor, enabled);
}
void camera_set_v_flip(uint8_t enabled) {
	s_sensor.set_vflip(&s_sensor, enabled);
}

void camera_set_exposure_ctrl(uint8_t enabled, uint16_t val) {
	s_sensor.set_exposure_ctrl(&s_sensor, enabled, val);
}
...
static void IRAM_ATTR signal_dma_buf_received(bool* need_yield)
{
    ...
    //ESP_EARLY_LOGW(TAG, "xQueueSendFromISR: ret=%d", ret);
    if (ret != pdTRUE) {
