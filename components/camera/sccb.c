...
uint8_t SCCB_Read_TwoBytesReg(uint8_t slv_addr, uint16_t reg)
{
	uint8_t reg_8[2] = {0x00,0x00};
    uint8_t data=0;
    ...
    __disable_irq();
    int rc = twi_writeTo(slv_addr, reg_8, 2, true);
    if (rc != 0) {
        data = 0xff;
    }
    else {
    ...
    __enable_irq();
    if (rc != 0) {
        printf("SCCB_Read [%02x, %02x] failed rc=%d\n", reg_8[0], reg_8[1], rc);
    }
    return data;
}
...
uint8_t SCCB_Write_TwoBytesReg(uint8_t slv_addr, uint16_t reg, uint8_t data)
{
    uint8_t ret=0;
    uint8_t buf[] = {reg>>8, reg&0x00FF, data};
    ...
    __enable_irq();
    if (ret != 0) {
        printf("SCCB_Write [%02x, %02x]=%02x failed\n", reg>>8, reg&0x00FF, data);
    }
    return ret;
}
